...
class Sales extends MY_Controller
{
	...
	function getSales($warehouse_id = NULL)
    {
       ...
       // echo $permission->product_edit;die();
        ...
        if (isset($warehouse_id)) {
            $this->datatables
                ->select("sales.id, sales.date, reference_no, sales.biller, customer, users.username AS saleman,delivery.username as delivery_man, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status")
                ->from('sales')
				->join('companies', 'companies.id = sales.customer_id', 'left')
				->join('users', 'users.id = sales.saleman_by', 'left')
				->join('deliveries', 'deliveries.sale_id = sales.id', 'left')
				->join('users as delivery', 'delivery.id = deliveries.delivery_by', 'left')
                ->where('sales.warehouse_id', $warehouse_id);
        } else {
			$this->datatables
				->select("sales.id, sales.date, sales.reference_no, sales.biller, companies.name AS customer, users.username AS saleman,delivery.username as delivery_man ,sales.sale_status, sales.grand_total, sales.paid, (grand_total-paid) as balance, sales.payment_status")
				->from('sales')
				->join('users', 'users.id = sales.saleman_by', 'left')
				->join('companies', 'companies.id = sales.customer_id', 'left')
				->join('deliveries', 'deliveries.sale_id = sales.id', 'left')
	...
	function invoice_landscap_a5_kh($id = NULL, $modal = NULL)
    {
		$this->erp->checkPermissions('index');
		...
		//echo $invoice_view;exit;
		if($invoice_view==0){
			//redirect("sales/print_/".$s->row()->id);
			$this->load->view($this->theme . 'sales/modal_print', $this->data);
		}
		else if($invoice_view==1){
			//redirect("sales/invoice/".$s->row()->id);
			$this->load->view($this->theme . 'sales/modal_invoice', $this->data);
		}
		else if($invoice_view==2){
			//redirect("sales/tax_invoice/".$s->row()->id);
			$this->load->view($this->theme . 'sales/modal_tax_invoice', $this->data);
		}
			...
			$this->load->view($this->theme . 'sales/invoice_landscap_a5_kh', $this->data);
		}else{
			$this->load->view($this->theme . 'sales/modal_print', $this->data);
		}
    }
    ...
    function add($quote_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
				...
				if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $products[] = array(
                        ...
                        'net_unit_price' => $this->erp->formatDecimal($item_net_price),
			...
			if($purchase_type==1 && $total_tax==0){
        ...
        if ($this->form_validation->run() == true) {
			...
			if($invoice_view==0){
				redirect("sales/print_lvs/".$s->row()->id);
			}
			else if($invoice_view==1){
				redirect("sales/invoice/".$s->row()->id);
			}
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
			...
			*/
            ...
            foreach ($inv_items as $item) {
                $row = $this->site->getProductByID($item->product_id);
				$group_price = $this->sales_model->getProductPriceGroup($item->product_id, $sale->customer_id);
                if (!$row) {
                    $row = json_decode('{}');
                    $row->tax_method = 0;
                    $row->quantity = 0;
...
//        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'combine') {					
                    $html = $this->multi_print($_POST['val']);
                }
	...
	function multi_print($sale_id = NULL)
    {
        ...
        $this->data['barcode'] = $this->barcode($inv->reference_no, 'code39', 30);        
        $this->data['page_title'] = $this->lang->line("invoice");
        $this->load->view($this->theme . 'sales/multi_print', $this->data);
    }
	...
	function delivery_add($id = NULL)
    {
		...
		if($this->form_validation->run() == true){
			...
			$i = isset($product_id) ? sizeof($product_id) : 0;
            for ($r = 0; $r < $i; $r++) {
				$item_id       = $_POST['product_id'][$r];
                $item_name     = $_POST['product_name'][$r];
                $warehouse     = $_POST['ware'][$r];
				$category_name = $_POST['catename'][$r];
				$price         = $_POST['price'][$r];
				$delivery_qty  = $_POST['delivery_qty'][$r];
			...
			//$this->erp->print_arrays($data);
			if($this->sales_model->addDeliveryItem($data)){
			...
			$this->data['page_title'] = lang("delivery_order");
			$this->data['modal_js'] = $this->site->modal_js();
			$this->load->view($this->theme . 'sales/delivery_add', $this->data);
		}
    }
	
	function delivery_update($id = NULL)
    {
        $this->erp->checkPermissions('deliveries');
		...
		if($this->form_validation->run() == true){
			...
			//$this->erp->print_arrays($data);
			if($this->sales_model->updateDeliveryItem($id, $data)){
				$this->session->set_flashdata('message', lang("delivery_added"));
				redirect($_SERVER["HTTP_REFERER"]);
			}else{
				$this->session->set_flashdata('error', validation_errors());
			...
			$this->data['page_title'] = lang("delivery_order");
			$this->data['modal_js'] = $this->site->modal_js();
			$this->load->view($this->theme . 'sales/delivery_update', $this->data);
		}
    }
        ...
        if ($this->form_validation->run() == true) {
			...
			foreach($sale_id_arr as $sale_id){
				...
				$payment = array(
					...
					'amount' => str_replace(',', '', $amount_paid_arr[$i]),
					'paid_by' => $this->input->post('paid_by'),
					'cheque_no' => $this->input->post('cheque_no'),
					'cc_no' => $this->input->post('paid_by') == 'gift_card' ? $this->input->post('gift_card_no') : $this->input->post('pcc_no'),
					'cc_holder' => $this->input->post('pcc_holder'),
	...
	function add_payment_loan($data = NULL,$id = NULL,$paid_amount = NULL,$principle = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$curr_paid = 0;
			$help = false;
			//$this->erp->print_arrays($balance_amount);
			for($i=0; $i<sizeof($arr_id)-1; $i++){
				...
				//if($this->sales_model->addPaymentLoan($loans)) {
					...
					$balance_amount -= $sale_loan->payment;
					$help = true;
				//}
			}
			if($balance_amount != 0) {
				$balance_amt = $sale_loan->balance - $balance_amount;
				$term = $this->sales_model->getTermBySaleID($sale_loan->sale_id);
				$new_term = $term - $sale_loan->period;
				$loan_id = $sale_loan->id + 1;
				$period = $sale_loan->period + 1;
				$rate = $sale_loan->rated;
				if($loan_id != '' && $sale_loan->sale_id != '' && $period != '' && $balance_amt > 0 && $rate > 0 && $new_term > 0) {
					$new_rate = $this->erp->getNewRate($loan_id, $sale_loan->sale_id, $period, $balance_amt, $rate, $new_term, $sale_loan->type);
					if($new_rate) {
						$this->sales_model->changeRates($new_rate);
					}
				}		
			}
			//$this->erp->print_arrays($new_rate);
			if($help) {
				$payments = array(
    ...
    function suggestions()
    {
        ...
        $rows = $this->sales_model->getProductNames($sr, $warehouse_id, $user_setting->sales_standard, $user_setting->sales_combo, $user_setting->sales_digital, $user_setting->sales_service, $user_setting->sales_category,$customer->price_group_id);
        if ($rows) {
            foreach ($rows as $row) {
                $option = FALSE;
                $row->quantity = 0;
				...
				$group_price = $this->sales_model->getProductPriceGroup($row->id, $customer->price_group_id);
                if ($options) {
                    $opt = $options[0];
                    if (!$option) {
                        $option = $opt->id;
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")". " (" . $row->price . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'group_price' => $group_price);
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")". " (" . $row->price . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'group_price' => $group_price);
                }
            }
            echo json_encode($pr);
        } else {
    ...
    function getSuspends_calendar()
    {
		...
		if ($reference_no) {
			$this->datatables->where('erp_suspended_bills.suspend_name', $reference_no);
		}
	...
	function print_lvs($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        ...
        $this->data['title'] = "2";
		$this->data['sid'] = $id;
        $this->load->view($this->theme.'sales/print_lvs',$this->data);
    }
	function print_pp($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        $this->erp->checkPermissions('add', true, 'sales');
        ...
        $this->data['title'] = "2";
		$this->data['sid'] = $id;
        $this->load->view($this->theme.'sales/print_pp',$this->data);
    }
}
