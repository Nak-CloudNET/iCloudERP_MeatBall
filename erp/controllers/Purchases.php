...
class Purchases extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$all_discount = $this->input->post('discount') ? $this->input->post('discount') : 0;
			...
			if($setting->accounting_method == 2 || $shipping){
				...
				for($i = 0; $i < $c; $i++){
					...
					$costunit = $this->site->calculateAverageCost($products[$i]['product_id'], $unitCost, $products[$i]['quantity'], $c, $products[$i]['item_discount'], $discounts, $shipping, $t_po_item_amount);
            ...
            if ($this->Settings->tax2 != 0) {
				if( $this->input->post('purchase_type')==2){
					$order_tax_id = $this->input->post('order_tax1');
				}else{
					$order_tax_id = $this->input->post('order_tax');
				}
    ...
    public function purchase_actions()
    {
		...
		*/
		if ($this->input->post('form_action') == 'undeclare'){
			$ids = $_POST['val'];
				foreach ($ids as $id) {
                       $this->purchases_model->updatePurchaseTaxStatusById($id);
					   echo $id.'<br/>';
               }
                    $this->session->set_flashdata('message', lang("purchasing_tax_undeclared"));
                   redirect($_SERVER["HTTP_REFERER"]);
			}
		if($this->input->post('form_action')=='declare'){
			$ids = $_POST['val'];
			foreach($ids as $purchase_id){
				$this->load->model('purchases_model');
				$combine_tax = $this->purchases_model->getCombinePurchaseTaxById($purchase_id);
				$sale_items = $this->purchases_model->getItemsByPurchaseId($purchase_id);
					foreach($sale_items as $item){
						$qty+=$item->quantity;
					}
				$purchase_type=$combine_tax->purchase_type;
				$purchase_id=$combine_tax->id;
				$vat=$combine_tax->total_tax;
				$amount=($combine_tax->grand_total)-$vat;
				$supplier_id= $combine_tax->supplier_id;
				$order_tax_id=$combine_tax->order_tax_id;
				$reference_no=$combine_tax->reference_no;
				$warehouse_id=$combine_tax->warehouse_id;
				$purchase_type=$combine_tax->purchase_type;
				if($purchase_type==1){
				...
				if($purchase_type==3){
					...
					$non_tax_purchase=0;
						...
						//echo 'VAT_D:'.$vat_declare.' |Amount_D: '.$amount_declare.' | Amount:'.$amount.' | VAT'.$vat.'| NoTax:'.$non_tax_purchase.'| Import:'.$value_import.' |Type:'.$purchase_type.'Tax_ID'.$combine_tax->order_tax_id.'<br/>';		
				...
				$purchase_tax = array(
								...
								'amount' =>$amount,
								'amount_tax' => $vat,
								'amount_declear'=>$amount_declare,
								'amount_tax_declare' => $vat_declare,
								'non_tax_pur' => $non_tax_purchase,
								'value_import' => $value_import,
								'reference_no' => $reference_no,
								'group_id' => $warehouse_id,
								'qty' => $qty,
								'purchase_type' => $purchase_type
							);	
						$this->purchases_model->declare_tax($purchase_tax);
					
					
				}
				$this->session->set_flashdata('message', lang("tax_added"));
				redirect('taxes/purchasing_tax','refresh');
			}
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPayment($payment)) {
            ...
            $purchase = $this->purchases_model->getPurchaseByID($id);
			$this->data['discount'] = $this->purchases_model->getDiscount($purchase->payment_term);
