...
class Taxes extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("id, date, ". $this->db->dbprefix("purchases").".reference_no, supplier,note, status, (grand_total - order_tax) as amount, (IF(tax_status<>'', amount_tax, order_tax)) as tax_declear, (grand_total) as balance,
				IF(erp_purchases.purchase_type = '1', 'Taxable Purchase', IF(erp_purchases.purchase_type = '2', 'Non Taxable Purchase','Import')) as remark,
				(IF(tax_status<>'', tax_status, '')) as status_tax")
                ->from('purchases')
	 ...
	 function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
				...
				$this->datatables
					->select("erp_sales.id,date, reference_no, biller, customer,note,total as balance,total_tax,grand_total,sale_tax.amound_declare,sale_tax.amount_tax_declare,(erp_sale_tax.amound_declare+erp_sale_tax.amount_tax_declare) as total_declare,
					IF(sale_type = '1', 'Taxable Sales', IF(sale_type = '2', 'Non Taxable Sales','Export')) as remark,tax_status", FALSE)
					->from('sales')
	...
	function tax_return_for_small_tax_payers()
	{
		...
		$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
		$this->data['modal_js'] = $this->site->modal_js();
		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('tax_return_for_small_tax_payers')));
		$meta = array('page_title' => lang('tax_return_for_small_tax_payers'), 'bc' => $bc);
