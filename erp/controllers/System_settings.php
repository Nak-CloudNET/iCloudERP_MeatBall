...
class system_settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('site_name' => DEMO ? 'iCloudERP - POS' : $this->input->post('site_name'),
				...
				'adjustment_prefix' => $this->input->post('quantity_adjustment_prefix'),
	...
	function getPaymentTerm(){
        ...
        $this->datatables
            ->select("id, description, payment_days, over_due_days, payment_date, discount")
            ->from("payment_term")
			->order_by('id', 'asc')
	...
	public function add_payment_term(){
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'payment_days' => $this->input->post('payment_day'),
                'over_due_days'=> $this->input->post('over_due_day'),
				'payment_date' => $this->erp->fsd($this->input->post('payment_date')),
	...
	public function delete_payment_term($id){
		if ($this->settings_model->deletePaymentTerm($id)) {
            ...
            redirect('system_settings/payment_term');
        }
	}
    ...
    function getWarehouses()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id,map,id as d, code, name, phone, email, address")
            ->from("warehouses")
			...
			if($id_convert_item != 0){
				...
				foreach($items as $item){
					...
					if($item->status == 'deduct'){
						...
						$this->db->update('bom_items', array('cost' => $each_cost), array('product_id' => $item->product_id, 'bom_id' => $item->bom_id));
					}
				}
			}
    ...
    function add_price_group()
    {
        ...
        $this->form_validation->set_rules('name', lang("group_name"), 'trim|is_unique[price_groups.name]');
    ...
    function update_product_group_price($group_id = NULL)
    {
		...
		//echo $currency_code;exit;
        if (!empty($product_id) && !empty($price)) {
            if ($this->settings_model->setProductPriceForPriceGroup($product_id, $group_id, $price, $currency_code)) {
    ...
    function add_customer_products()
    {
    ...
    function edit_customer_products($id = NULL)
    {
    ...
    function delete_customer_products($id = NULL)
    {
        if ($this->settings_model->deletePriceGroup($id)) {
            echo lang("price_group_deleted");
        }
    ...
    function customer_products_group_actions($group_id)
    {
        if (!$group_id) {
            $this->session->set_flashdata('error', lang('no_price_group_selected'));
            redirect('system_settings/price_groups');
    ...
    function customer_products_group($group_id = NULL)
    {
    ...
    function getCustomerProductsGroup($group_id = NULL)
    {
        if (!$group_id) {
            $this->session->set_flashdata('error', lang('no_price_group_selected'));
            redirect('system_settings/price_groups');
    ...
    function update_customer_products_group($group_id = NULL)
    {
        if (!$group_id) {
            $this->erp->send_json(array('status' => 0));
        }
