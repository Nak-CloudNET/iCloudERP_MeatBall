...
class Pos extends MY_Controller
{
	...
	function combine_payment()
    {
        ...
        $this->form_validation->set_rules('amount_paid', lang("amount"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->load->view($this->theme . 'pos/combine_payment', $this->data);
		}
    }
	...
	function print_complete($id){
		$this->erp->checkPermissions('index');
		$this->data['data'] = $this->pos_model->printComplete($id);
		$this->load->view($this->theme . 'pos/print_complete', $this->data);
	}
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
			$this->datatables
				...
				".$this->db->dbprefix('sales').".date, (max(
				".$this->db->dbprefix('payments').".date)) as pdate,
				".$this->db->dbprefix('sales').".reference_no, customer, sale_status, total_items, grand_total, paid, (grand_total - paid) AS balance, payment_status")
                ->from('sales')
			...
			if(isset($_REQUEST['d'])){
				...
				$this->datatables
						->where("erp_sales.due_date >=", $date)
						->where('erp_sales.due_date <= CURDATE()')
		...
		if($start_date){
			$this->datatables->where($this->db->dbprefix('sales').'.date BETWEEN "' .$this->erp->fld($start_date).'" and "'.$this->erp->fld($end_date).'"');
		}
		if($payment_status=='All'){
		   redirect($_SERVER["HTTP_REFERER"]);
		}elseif($payment_status=='partial' or $payment_status=='due' or $payment_status=='paid'){
			$this->datatables->where($this->db->dbprefix('sales.payment_status'),$payment_status);
		}
    ...
    function index($sid = null)
    {
        ...
        if (!$this->pos_settings->default_biller || !$this->pos_settings->default_category) {
        ...
        //validate form input
        $this->form_validation->set_rules('reference_nob', lang("reference_no"), 'trim|is_unique[sales.reference_no]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment_status 	= 'due';
			$get_customer_term 	= $this->pos_model->getDayPaymentTerm($customer_id);
            $payment_term 		= $get_customer_term->day_payment_term;
			$term_date 			= date('Y-m-d', strtotime($date));
            $due_date 			= $payment_term ? date('Y-m-d', strtotime($term_date. '+' . $payment_term . ' days')) : NULL;
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $products[] = array(
                        ...
                        'unit_price' => $this->erp->formatDecimal($item_net_price + $pr_discount + $item_tax),
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
						...
						if(strpos($_POST['amount'][$r], '-') !== false){
							$payment[] = array(
								'biller_id'	=> $biller_id,
								'date' => $payment_date ? $payment_date : $date,
				...
				if(isset($p_cur) && empty($_POST['amount'][0])){
					...
					for ($j = 0; $j < $p_cur; $j++) {
						$payment[] = array(
							...
							'pos_balance' => $this->erp->formatDecimal(($_POST['other_cur_paid'] / $cur_rate->rate) - $paidd),
							'pos_paid_other' => $_POST['other_cur_paid']
						);
					}
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
				...
				if ($sale = $this->pos_model->addSale($data, $products, $payment,$did, $loans)) {
                    ...
                    //redirect("pos/print_lvs/" . $sale['sale_id']);
					redirect("pos");
                }
            }
        }
			...
			$this->data['reference'] = $this->site->getReference('pos');
	...
	/*function print_lvs($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        ...
        $this->data['title'] = "2";
		$this->data['sid'] = $id;
        $this->load->view($this->theme.'pos/print_lvs',$this->data);
    }*/
	function print_lvs($sale_id = NULL, $modal = NULL)
    {
    ...
    function close_register($user_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['due_payments'] = $this->pos_model->getRegisterDuePayment($register_open_time, $user_id);
    ...
    function getProductDataByCode($code = NULL, $warehouse_id = NULL, $cust_id = null, $suspend_id = null, $item_rows = null, $sub_total = null)
    {
		...
		$row = $this->pos_model->getWHProduct($code, $warehouse_id,$customer->price_group_id);
        $option = '';
        if ($row) {
            $combo_items = FALSE;
			...
			$group_price = $this->pos_model->getProductPriceGroup($row->id);
            if ($options) {
                $option_quantity = 0;
                foreach ($options as $option) {
                    $pis = $this->pos_model->getPurchasedItems($row->id, $warehouse_id, $row->option);
            ...
            if ($row->tax_rate) {
                ...
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'item_price' => $row->price, 'group_price' => $group_price);
            } else {
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'item_price' => $row->price, 'group_price' => $group_price);
            }
	...
	function getAjaxReferenceByRef($ref = NULL, $date = NULL)
    {
		if($ref){
			$get_date = explode('_', $date);
			$exp_date = explode('-', $get_date[0]);
			$ref_date = $exp_date[0].'-'.$exp_date[1];
			$row = $this->pos_model->getSalesRef($ref, $ref_date);
			if ($row) {
				echo 1;
			} else {
				echo 0;
	...
	function view_kh($sale_id = NULL, $modal = NULL)
    {
        $this->erp->checkPermissions('index');
        if ($this->input->get('id')) {
            $sale_id = $this->input->get('id');
        ...
        $this->data['modal'] = $modal;
        $this->data['page_title'] = $this->lang->line("invoice");
        $this->load->view($this->theme . 'pos/view_price_kh', $this->data);
    }
    ...
    function settings()
    {
        ...
        $this->form_validation->set_rules('customer', $this->lang->line('default_customer'), 'required');
        ...
        if ($this->form_validation->run() == true && $this->pos_model->updateSetting($data)) {
            ...
            $this->data['customers'] = $this->pos_model->getAllCutomerCompanies();
	...
	function view_order($per_page = 0){
		$this->load->library('pagination');
        ...
        $config['base_url'] = site_url('pos/view_order');
        $config['total_rows'] = $this->pos_model->orders_count();
        $config['per_page'] = 10;
        $config['num_links'] = 3;
		...
		$this->data['data'] = $this->pos_model->getOrder($config['per_page'], $per_page);
		$this->load->view($this->theme . 'pos/view_order', $this->data);
	}
	...
	function deleteOrderitem($id){
		$delete = $this->pos_model->clearOrderitem($id);
		if($delete){
			redirect($_SERVER["HTTP_REFERER"]);
		}
	}
	...
	function addOrderitem(){
		...
		foreach($exp as $id){
			...
			$res = $this->db->insert('sale_items',$products);
		}
		redirect($_SERVER["HTTP_REFERER"]);
	}
	...
	function addDeliveryitem(){
		...
		$i = 0;
		foreach($exp as $id){
			$rows = $this->site->getSaleItems($id); 
			$row = $this->pos_model->getDeliveryrefer($refer); 
			$data = array(
				'do_reference_no' => $row->do_reference_no,
			...
			$i++;
			//print_r($data);
			$this->pos_model->addDeliveryItem($data);
		}

	}
	
	function view_order_print($id){
		$this->data['data'] = $this->pos_model->getDeliveryItemById($id);
		$this->load->view($this->theme . 'pos/view_order_print', $this->data);
	}
