...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $price_group_id = NULL, $limit = 15)
    {
		if($price_group_id > 0){
			$this->db->select('products.id, code, name, type, cost, warehouses_products.quantity, 
				(CASE WHEN (SELECT COALESCE(erp_product_prices.price, 0) 
					FROM erp_product_prices 
					WHERE price_group_id = "'. $price_group_id .'"
					AND product_id = erp_products.id
				) > 0 THEN (SELECT COALESCE(erp_product_prices.price, 0) 
					FROM erp_product_prices 
					WHERE price_group_id = "'. $price_group_id .'"
					AND product_id = erp_products.id
				) ELSE price END) AS price , 
				(SELECT IFNULL(currency_code, "USD") AS currency_code
					FROM erp_products p
					LEFT JOIN erp_product_prices ON erp_product_prices.product_id = p.id
					WHERE price_group_id = "'. $price_group_id .'"
					AND product_id = erp_products.id
				) AS currency_code,
				tax_rate, tax_method, image, promotion, promo_price, product_details, details, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $limit = 5)
    {
		/*if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
			$this->db->select('products.id, code, name, type, image, cost,warehouses_products.quantity, price, tax_rate, tax_method, product_details, details, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
					FROM erp_serial as sp
				 WHERE sp.product_id='.$this->db->dbprefix('products').'.id
				), "") as sep')
            ->group_by('products.id');
		...
		}else
		{
			...
			$this->db->select('products.id, code, name, type, cost, image, warehouses_products.quantity, price, tax_rate, tax_method, product_details, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
					FROM erp_serial as sp
				 WHERE sp.product_id='.$this->db->dbprefix('products').'.id
				), "") as sep')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
	...
	public function getProductCodes($term, $warehouse_id, $limit = 5)
    {
		$this->db->select($this->db->dbprefix('products').'.id,
				...
				details, category_id, price, tax_method, tax_rate, type,
	...
	/* POS Price Group */
	public function getProductPriceGroup($id, $group_id){
		$where = '';
		if($group_id){
			$where = array('product_id' => $id, 'price_group_id' => $group_id);
		}else{
			$where = array('product_id' => $id);
		}
		$q = $this->db->get_where('product_prices', $where);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.details as details, product_variants.name as variant,units.name as unit, products.promotion, categories.name AS category_name')
    ...
    {
        $this->db->select('sale_items.id, sale_items.product_name, sale_items.product_code,products.price, sale_items.quantity, sale_items.serial_no, sale_items.tax, sale_items.net_unit_price, sale_items.item_tax, sale_items.item_discount, sale_items.subtotal, products.details, sale_items.product_id, sale_items.warehouse_id as ware, products.category_id as catename, sale_items.option_id as variant');
	...
	public function getExportInvoiceByID($id)
    {
        $this->db->select('sales.id, sales.date as date, payments.date as pdate, sales.reference_no, sales.customer, sales.grand_total, sales.total_items, sales.biller, sales.sale_status, sales.payment_status, sales.paid');
		$this->db->from('sales');
	...
	public function getLoanByID($id = NULL) {
	...
	public function addSale($data = array(), $items = array(), $payment = array(), $loans = array())
	{
		...
		foreach($items as $g){
			$type = $this->site->getProductByID($g['product_id']);
			if($type->type == 'combo'){
				$totalCostProducts = $this->getProductCostByComboitems($g['product_id'], $g['quantity']);
			}else{
	...
	public function updateDeliveryItem($id, $data){
		...
		foreach($data as $item){
			...
			$this->db->update('delivery_items', $item, array('id' => $id[$i]) );
			$i++;
		}
		return false;
	}
    ...
    {
		$cost = $this->site->costing($items, $id);
        ...
        if ($this->db->update('sales', $data, array('id' => $id)) && $this->db->delete('sale_items', array('sale_id' => $id))) {
			...
			$this->db->delete('purchase_items', array('sale_id'=>$id));
			foreach ($items as $item) {
				$this->db->insert('sale_items', $item);
                $sale_item_id = $this->db->insert_id();
                if ($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {
                    $item_costs = $this->site->item_costing($item, true, $id);
                    foreach ($item_costs as $item_cost) {
                        $item_cost['sale_item_id'] = $sale_item_id;
	...
	public function getProductCostByComboitems($product_id, $quantity){
		$this->db->select("SUM(erp_products.cost* CASE WHEN $quantity <> 0 THEN $quantity ELSE 0 END ) AS total_cost ");
		$this->db->join('products', 'products.code = combo_items.item_code');
	...
	public function getTermBySaleID($id = NULL) {
		$q = $this->db->select('MAX(period) as term')
					  ->get_where('loans', array('sale_id' => $id));
		if($q->num_rows() > 0) {
			return $q->row()->term;
		}
		return FALSE;
	...
	public function changeRates($data = array()) {
		$help = true;
		if($data) {
			foreach($data as $dt) {
				$id = $dt['id'];
				...
				unset($dt['sale_id']);
				unset($dt['period']);
				if($this->db->update('loans', $dt, array('id' => $id, 'sale_id' => $sale_id, 'period' => $period))) {
					$help = false;
				}
			}
		}
