...
class Taxes_model extends CI_Model
{
    ...
    {
		$this->db->select('id, date, reference_no, biller, customer,total_tax,warehouse_id,order_tax,order_tax_id, sale_status,grand_total,(grand_total-total_tax) as balance,sale_type');
		$this->db->from('sales');
		$this->db->where_in('id', $id);
        $q = $this->db->get();
         if ($q->num_rows() > 0) {
