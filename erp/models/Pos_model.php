...
class Pos_model extends CI_Model
{
	...
	public function getSalesRef($ref, $date)
    {
        $this->db->select('reference_no');
		$q = $this->db->get_where('sales', array('reference_no' => $ref, 'DATE_FORMAT(date, "%Y-%m") =' => $date), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    }
	...
	*/
    ...
    public function getWHProduct($code, $warehouse_id, $price_group_id = NULL)
    {
		/*
        $this->db->select('products.id, code, name, type, warehouses_products.quantity, price, tax_rate, tax_method,image, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
	...
	public function getAllCutomerCompanies()
    {
	...
	public function getDayPaymentTerm($id)
    {
        $this->db->select('day_payment_term');
    ...
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, units.name as unit')
    ...
    */
	...
	public function getRegisterDuePayment($date, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('erp_sales.date as sale_date, SUM( COALESCE(erp_payments.amount, 0 ) ) as grand_total', FALSE)
            ->join('sales', 'sales.id = payments.sale_id', 'left')
            ->where('payments.type', 'received')
            ->where('payments.date >', $date)
			->where('sales.date <', $date)
	...
	public function printComplete($id){
		$this->db->select('erp_suspended_items.id as idd, product_code, product_name, erp_products.image, erp_suspended_items.quantity, erp_suspended_bills.suspend_name as table, erp_suspended_items.unit_price as price')
				 ->from('suspended_items')
				 ->join('products', 'products.id = suspended_items.product_id', 'left')
				 ->join('suspended_bills', 'suspended_bills.id = suspended_items.suspend_id', 'left')
				 ->where('erp_suspended_bills.suspend_id', $id);
		$q = $this->db->get();
		if ($q->num_rows() > 0) {
            return $q->result();
        }
	...
	public function clearOrderitem($id){
		$delete = $this->db->delete('sale_items', array('id' => $id));
		if($delete){
			return true;
		}
		return false;
	...
	public function addDeliveryItem($data){
		if($this->db->insert('delivery_items', $data)){
			return true;
		}
		return false;
	}
	
	public function getDeliveryItemById($id){
	...
	public function getDeliveryrefer($refer){
		$q = $this->db->get_where('deliveries', array('sale_reference_no' => $refer), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
