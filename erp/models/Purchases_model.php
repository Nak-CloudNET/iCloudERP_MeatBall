...
class Purchases_model extends CI_Model
{
		...
		if ($this->db->insert('purchases', $data)) {
            ...
            foreach ($items as $item) {
				...
				if($data['status'] == 'received' || $data['status'] == 'pending'){
					$this->db->update('products', array('cost' => $item['real_unit_cost'], 'price'=>$price, 'quantity' => $item['quantity']), array('id' => $item['product_id']));
				}
    ...
    public function addPayment($data = array())
    {
		...
		if ($this->db->insert('payments', $data)) {
			...
			$deposit = $this->site->getDepositByCompanyID($purchase->supplier_id);
			$deposit_balance = $deposit->deposit_amount;
			$deposit_balance = $deposit_balance - abs($data['amount']);
			if($this->db->update('companies', array('deposit_amount' => $deposit_balance), array('id' => $purchase->supplier_id))){
				$this->db->update('deposits', array('amount' => $deposit_balance), array('company_id' => $purchase->supplier_id));
			}
	...
	public function getCombinePurchaseTaxById($id)
    {
		$q = $this->db->get_where('purchases', array('id'=> $id), 1);
		if($q->num_rows() > 0){
            return $q->row();
	...
	public function declare_tax($data)
    {
        if ($this->db->insert('purchase_tax', $data)) {
			$this->db->update('purchases', array('reference_no_tax' => $data['referent_no'],'tax_status'=>'confirmed'), array('id' => $data['purchase_id']));
	...
	public function updatePurchaseTaxStatusById($id){
		$data = array(
               'tax_status' => NULL
            );
		
				$this->db->where('id',$id);
				$u=$this->db->update('purchases',$data);
				$this->db->where('purchase_id',$id);
				$d=$this->db->delete('purchase_tax');
