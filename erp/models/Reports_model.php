...
ORDER BY
    ...
    {
        $myQuery = "SELECT COUNT(id) as total_invoice, DATE_FORMAT( date,  '%e' ) AS date, SUM( COALESCE( total_items, 0 ) ) AS total_items, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( total_discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
            FROM " . $this->db->dbprefix('sales') . "
    ...
    {
        $myQuery = "SELECT COUNT(id) AS total_invoice, DATE_FORMAT( date,  '%c' ) AS date, date as sale_Date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( total_items, 0 ) ) AS total_items, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( total_discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
            FROM " . $this->db->dbprefix('sales') . "
    ...
    {
        $this->db->select('SUM( COALESCE( erp_products.cost * (erp_sale_items.quantity - COALESCE(erp_return_items.quantity, 0)), 0 ) ) AS cost, SUM( COALESCE( grand_total, 0 ) ) AS sales, SUM( total_tax + shipping + total_cost ) AS net_cost, SUM( total_tax + shipping + grand_total ) AS net_sales', FALSE)
    ...
    {
        $this->db->select('SUM( COALESCE( total_items, 0 ) ) AS total_items, SUM( COALESCE( erp_products.cost * (erp_sale_items.quantity - COALESCE(erp_return_items.quantity, 0)), 0 ) ) AS cost, SUM( COALESCE( grand_total, 0 ) ) AS sales, SUM( total_tax + shipping + total_cost ) AS net_cost, SUM( total_tax + shipping + grand_total ) AS net_sales, SUM(COALESCE(erp_sale_items.subtotal, 0 ) ) AS no_total', FALSE)
            ->join('sale_items','sale_items.sale_id = sales.id', 'left')
            ->join('products','sale_items.product_id = products.id', 'left')
            ->join('return_items','return_items.sale_item_id = sale_items.id', 'left');
        if($date) {
    ...
    public function saleStockDetail($product_code)
    {
        $this->db->order_by('sale_items.id', 'asc');
        $q = $this->db->get_where('sale_items', array('product_code' => $product_code));
        if ($q->num_rows() > 0) {
    ...
    public function getSaleStockDetail($product_code)
    {
    ...
    public function getPurchaseDetail($product_code)
    {
        ...
        $this->db->where('purchase_items.opening_qty =', '1');
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getProductStockDetail($product_code)
    {
        $this->db->select('*');
        $this->db->from('purchase_items');
        $this->db->where('purchase_items.product_code', $product_code);
    ...
    */
    ...
    public function getPaymentsByID($id){
        $this->db
            ->select($this->db->dbprefix('payments') . ".id as idd, ". $this->db->dbprefix('sales') . ".date as sale_date, ". $this->db->dbprefix('payments'). ".date, " . $this->db->dbprefix('payments') . ".reference_no as payment_ref, " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, " . $this->db->dbprefix('sales') . ".customer as customer,  " . $this->db->dbprefix('sales') . ".total_items, " . $this->db->dbprefix('payments') . ".note,paid_by,amount, type")
            ->from('payments')
    ...
    public function getCustomersBalanceByID($id){
        $this->db
            ->select($this->db->dbprefix('companies') . ".id as idd, company, name, phone, email, count(" . $this->db->dbprefix('sales') . ".id) as total, COALESCE(sum(grand_total), 0) as total_amount, COALESCE(sum(paid), 0) as paid, ( COALESCE(sum(grand_total), 0) - COALESCE(sum(paid), 0)) as balance", FALSE)
            ->from("erp_companies")
    ...
    public function getSalesExportByID($id){
        ...
        $this->db
            ->select("erp_sales.id, date, reference_no, biller, customer, 
             GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name SEPARATOR '\n') as iname,
             GROUP_CONCAT(".$this->db->dbprefix('sale_items') . ".quantity SEPARATOR '__\n') as iqty, 
             GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".unit_price SEPARATOR '__\n' ) as iprice,
             GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".subtotal SEPARATOR '__\n' ) as subtotal,
             grand_total, paid, (grand_total-paid) as balance, paid as cost, paid as profit_lost, payment_status", FALSE)
    ...
    public function getSalesReportExportByID($id){
        $this->db
            ->select("erp_sales.id, sales.date, payments.date as pdate, sales.reference_no, sales.biller, sales.customer,
                            GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '(', " . $this->db->dbprefix('sale_items') . ".product_code , ')') SEPARATOR '\n') as iname, 
                            GROUP_CONCAT(CONCAT(((".$this->db->dbprefix('sale_items') . ".quantity)), '__', " . $this->db->dbprefix('sale_items') . ".unit_price) SEPARATOR '__') as iqty, 
                            GROUP_CONCAT(" . $this->db->dbprefix('products') . ".cost SEPARATOR '__') as icost, 
                            sales.grand_total, sales.paid, 
                            (grand_total-paid) as balance, 
                            payment_status, 
    ...
    function salemandaily($user_id, $date){
        $this->db->select($this->db->dbprefix('sales').".id as idd, date, reference_no, biller, customer,GROUP_CONCAT(CONCAT( '[', " . $this->db->dbprefix('sale_items') . ".product_code, ']', ' - ', CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity)) SEPARATOR '___') as iname, grand_total, paid, (grand_total-paid) as balance, payment_status", FALSE)
            ->from('sales')
            ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
            ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
            ->join('companies', 'companies.id=sales.customer_id','left')
            ->join('customer_groups','customer_groups.id=companies.customer_group_id','left')
            ->group_by('sales.id')
            ->where("saleman_by = {$user_id} AND DATE_FORMAT( date,  '%Y-%m-%d' ) =  '{$date}'");
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->result();
        }
    ...
    function getSalemanDailyByID($id){
        $this->db->select("date, reference_no, biller, customer,GROUP_CONCAT(CONCAT( '[', " . $this->db->dbprefix('sale_items') . ".product_code, ']', ' - ', CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity)) SEPARATOR '\n') as iname, grand_total, paid, (grand_total-paid) as balance, payment_status", FALSE)
            ->from('sales')
            ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
            ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
            ->join('companies', 'companies.id=sales.customer_id','left')
