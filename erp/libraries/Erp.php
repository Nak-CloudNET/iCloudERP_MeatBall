...
class Erp
{
	...
	function cutStringQty($str)
	{
		$ch = explode('__', $str);
		$get_qty = '';
		for($i = 0 ; $i <= sizeof($ch)-1; $i+=2){
			$get_qty[$i] =  $this->erp->formatQty($ch[$i]).' '.$ch[$i+1];
		}
		$return_qty = implode("\n", $get_qty);
		return $return_qty;
	}
	
	function cutStringQtyAndPrice($str)
	{
		$ch = explode('__', $str);
		$get_qty = '';
		for($i = 0 ; $i <= sizeof($ch)-1; $i+=2){
			$get_qty[$i] =  $this->erp->formatQty($ch[$i]).' ('.$this->erp->formatMoney($ch[$i+1]).'  ៛' .')';
		}
		$return_qty = implode("\n", $get_qty);
		return $return_qty;
	}
	...
	public function formatMoneyNoincrease($number){
		$db = explode('.',$number);
		$decimals = $this->Settings->decimals;
		return intval($number).'.'.substr(end($db),0,$decimals);
	}
	...
	function getNewRate($id = NULL, $sale_id = NULL, $period = NULL, $amount = NULL, $rate = NULL, $term = NULL, $p_type = NULL){
		$new_rate = array();
		if($p_type == 1){
			$principle = $amount/$term;
			$interest = 0;
			$balance = $amount;
			$payment = 0;
			for($i=0; $i<$term; $i++){
			...
			$interest = 0;
			$balance = $amount;
			$rate_amount = (($rate/100)/12);
			$payment = (($amount * $rate_amount)*((pow((1+$rate_amount),$term))/(pow((1+$rate_amount),$term)-1)));
			for($i=0; $i<$term; $i++){
				if($i== 0){
