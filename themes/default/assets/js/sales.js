...
function loadItems() {
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			var product_id = item.row.id, item_type = item.row.type, item_promotion = item.row.promotion, item_pro_price = item.row.promo_price, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_note = item.row.note, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;"), sep = item.row.sep;
			...
			item_price = item_tax_method == 0 ? formatDecimal(unit_price, 4) : formatDecimal(unit_price);
			...
			*/
			...
			tr_html += '<td><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
