...
function currencyFormat(x) {
    if (x != null) {
        return '<div class="text-center">'+formatMoney(x)+'</div>';
    } else {
        return '<div class="text-center">0</div>';
    }
}
...
$(document).ready(function() {
	...
	$('body').on('click', '.category_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'reports/getCategory/' + $(this).parent('.category_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
	...
	$('body').on('click', '.receipt_link_a5 td:not(:first-child, :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'sales/invoice_landscap_a5_kh/' + $(this).parent('.receipt_link_a5').attr('id') + '/1'});
        $('#myModal').modal('show');
    });
	...
	$('body').on('click', '.supplier_details_link td:not(:first-child)', function() {
