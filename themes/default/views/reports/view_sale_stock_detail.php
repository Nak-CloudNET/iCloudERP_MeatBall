...
<script>
    $(document).ready(function () {
        var oTable = $('#SlRData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('reports/getSaleStockDetail/'.$sale_details->product_code) ?>',
			...
			"bAutoWidth": false,
            "aoColumns": [null, null, {"mRender": fld}, null, {"mRender": currencyFormat}, {"mRender": formatQuantity}, {"mRender": currencyFormat}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var unit_cost = 0, qty = 0, cogs = 0;
                for (var i = 0; i < aaData.length; i++) {
						unit_cost += parseFloat(aaData[aiDisplay[i]][4]);
						qty += parseFloat(aaData[aiDisplay[i]][5]);
						cogs += parseFloat(aaData[aiDisplay[i]][6]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[4].innerHTML = currencyFormat(parseFloat(unit_cost));
                nCells[5].innerHTML = formatQuantity(parseFloat(qty));
                nCells[6].innerHTML = currencyFormat(parseFloat(cogs));
            }
            ...
            {column_number: 3, filter_default_label: "[<?=lang('customer');?> (yyyy-mm-dd)]", filter_type: "text", data: []},   
        ], "footer");
    });
</script>
