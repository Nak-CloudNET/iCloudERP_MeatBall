			...
			$(document).ready(function () {
				...
				var oTable = $('#SlRData').dataTable({
					...
					"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
						var qtotal = 0, gtotal = 0, paid = 0, balance = 0,costs=0,profit=0;
						for (var i = 0; i < aaData.length; i++) {
							var quantity = (aaData[aiDisplay[i]][6]);
							if(quantity != null) {
								if(quantity.indexOf('___') != -1){
									var qtotals = quantity.split('___');
									for(var j =0; j< qtotals.length; j++) {
										qtotal += parseFloat(qtotals[j]);
									}
								} else {
						...
						nCells[6].innerHTML = formatQuantity(parseFloat(qtotal));
...
<?php if ($Owner) {
			echo form_open('reports/customer_report_sales_actions', 'id="action-form"');
		} ?>
    ...
    <div id="payments-con" class="tab-pane fade in">
        ...
        $(document).ready(function () {
            ...
            var oTable = $('#PayRData').dataTable({
                ...
                "aoColumns": [{"bSortable": false, "mRender": checkbox}, {"mRender": fld}, null, null,{"bVisible": false}, {"mRender": formatQuantity}, null, {"mRender": paid_by}, {"mRender": currencyFormat}, {"mRender": row_status}],
                ...
                "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                    ...
                    var nCells = nRow.getElementsByTagName('th');
                    nCells[4].innerHTML = formatQuantity(parseFloat(qty));
