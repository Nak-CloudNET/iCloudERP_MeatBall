                        ...
                        <div id="print">
                            ...
                            <div id="left-bottom">
                                ...
                                <div id="botbuttons" style="text-align:center;">
									...
									<input type="hidden" name="saleman_1" id="saleman_1" value="<?= $this->session->userdata('user_id')?>"/>
									<input type="hidden" name="delivery_by_1" id="delivery_by_1" value="<?= $this->session->userdata('user_id')?>"/>
									
									<input type="hidden" name="sale_status" id="sale_status_1" value=""/>
									<input type="hidden" name="payment_term" id="payment_term_1" value=""/>
									...
									<input type="hidden" name="payment_date" value="" class="td_date">
									
                                    <div class="btn-group btn-group-justified">
                                        <div class="btn-group">
                                            <div class="btn-group btn-group-justified">
                                                <div class="btn-group">
                                                    <button type="button" style="height:68px;" class="btn btn-danger" id="reset">
                                        ...
                                        <div class="btn-group">
                                            <div class="btn-group btn-group-justified">
                                                <div class="btn-group">
                                                    <button type="button" style="height:68px;" class="btn btn-warning addButton" id="search_details">
                    					...
                    					<div class="btn-group">
                                        	<div class="btn-group btn-group-justified">
                                           		<div class="btn-group">                                                    											    	
													<button type="button" style="height:68px;" class="btn btn-warning open-suspend" id="suspend">
...
<?php } ?>
	<?php if($pos_settings->show_suspend_bar){
		...
		if(isset($arrSuspend[$sid]['suspend_not'])){
			$this->db->where('id',$arrSuspend[$sid]['suspend_not']);
		}
	...
	?>
    <button type="button" id="suspend" class="btn btn-danger open-suspend"><?= lang('suspend') ?>&nbsp;<?=(isset($arrSuspend[$sid]['suspend_not']) ? '<b>#<span class="sus_name">'.$namef.'</span></b>' : '')?></button>
	<?php } ?>
		...
		if($pos_settings->show_suspend_bar == 40){
			if($suspend->inactive == 1){
				...
				echo "<span style='position:relative;display: inline-table;'>
					<a id='clear_suspend' hrefs='".base_url()."pos/delete_suspend/".$suspend->ids."' style='text-decoration:none;position:absolute;bottom:5%;left:0;margin-left:3px;' class='btn-danger clear_suspend'>clear</a>
                            ...
                            <div class="row">
								...
								<div class="col-sm-6">
									<div class="form-group">
										...
										<?php $sst = array('completed' => lang('completed'));
									...
									<table class="table table-bordered table-condensed table-striped" style="font-size: 1.2em; font-weight: bold; margin-bottom: 0;">
										<tbody>
											...
											<tr>
												...
												?>
														<td class="text-right">
															<input name="other_cur_paid[]" rate="<?=$row->rate?>" type="text" id="other_cur_paid" class="form-control input-lg kb-pad currencies_payment" style="text-align:right;"/>
														</td>
												<?php
													}
												?>
											...
											<tr>
												<td width="50%" style="height: 50px;"><?= lang("remaining"); ?></td>
												<td class="text-right"><span id="remain">0.00</span></td>
												<!--
												<?php
													foreach ($query as $row)
													{
											...
											<tr>
												<td width="30%" style="height: 50px;"><?= lang("change"); ?></td>
...
-->									
										<div class="col-sm-6">
											<div class="form-group">
                                                <label style="font-size:13px;"><?= lang("paying_by") ?></label>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" id="pr_popover_content">
                <form class="form-horizontal" role="form">
                    ...
                    <?php } ?>
					<div class="form-group">
                        <label for="pcost" class="col-sm-4 control-label"><?= lang('cost') ?></label>

                        <div class="col-sm-8">
                            <input type="text" class="form-control kb-pad" id="pcost" disabled>
                        </div>
                    </div>
            ...
            <div class="modal-body" id="pr_popover_content">
                <table class="table table-bordered table-striped table-condensed table-hover"
                    ...
                    <tr>
						...
						<td>
							<button type="button" class="btn bdarkGreen col-md-12 addButton" id="opened_bills"> <?= $pos_settings->open_hold_bills ?></button>
						</td>
                    </tr>
                    <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$('#poscustomer').click(function(){
			$.ajax({
				type: "get", async: false,
				url: "<?= site_url('pos/getProductHistory') ?>/" + $('#poscustomer').val(),
				dataType: "json",
				success: function (data) {
					localStorage.setItem('positems', JSON.stringify(data)); 
					if(data){
		...
		$(document).on('change', '#saleman', function () {
            $('#saleman_1').val($(this).val());
        });
		$(document).on('change', '#delivery_by', function() {
		...
		$('#slpayment_term').live('change keyup paste', function() {
			$('#payment_term_1').val($(this).val());
		}).trigger('change');
        ...
        function autoCalcurrencies(total_p){
            $(".curr_tpay").each(function(){
                ...
                $(this).html(formatDecimal(total_p*rate));
            });
        }
		...
		function autoCalchange(total_p){
            $(".curr_change").each(function(){
                var rate = $(this).attr('rate');				
				if(total_p != 0){
					$(this).html(formatDecimal(parseInt(formateUSD(total_p)*rate)));
				}else{
					$(this).html('0');
				}
            });
		...
		$(document).on('change', '.deposit_no', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data !== null) {
						...
						var suspend_html = '<p>Number '+ table_no +'</p>';
		...
		*/
				...
				if(rate == '' || rate < 0) {
					...
					if(term == '' || term <= 0) {
						...
						if(p_type == 1 || p_type == 3 || p_type == 4){
							tr += '<tr>';
							tr += '<th> <?= lang("Pmt No."); ?> </th>';
							tr += '<th> <?= lang("interest"); ?> </th>';
							tr += '<th> <?= lang("principal"); ?> </th>';
							tr += '<th> <?= lang("total_payment"); ?> </th>';
							tr += '<th> <?= lang("balance"); ?> </th>';
							tr += '<th> <?= lang("note"); ?> </th>';
							tr += '<th> <?= lang("payment_date"); ?> </th>';
							tr += '</tr>';
						}else if(p_type == 2){
							tr += '<tr>';
							tr += '<th> <?= lang("period"); ?> </th>';
							tr += '<th> <?= lang("rate"); ?> </th>';
							tr += '<th> <?= lang("percentage"); ?> </th>';
							tr += '<th> <?= lang("payment"); ?> </th>';
							tr += '<th> <?= lang("total_payment"); ?> </th>';
							tr += '<th> <?= lang("balance"); ?> </th>';
							tr += '<th> <?= lang("note"); ?> </th>';
							tr += '<th> <?= lang("dateline"); ?> </th>';
							tr += '</tr>';
						}
						if(p_type == 1){
		...
		$(document).on('keyup','#amount_1, .currencies_payment', function(){
			...
			if(balance > 0){
				...
				$('#change').text(formateUSD(balance));
				$('#remain').css('color','black');
				$('.curr_remain').css('color','black');
				$('#remain').text('0.00');
			}else{
				autoCalremain(0);
				autoCalchange(0);
		...
		$(document).on('keyup','#tbl_dep .payment_amt', function () {
			var rate_all = $('#depreciation_rate_1').val()-0;
			var amount = 0;
			var percent = 0;
			var amount_payment = 0;
			...
			if(payment < 0 ) {
				...
				tr.find('.total_payment').val(formatDecimal(amount_payment));
				tr.find('.total_payment_').val(amount_payment);
				tr.find('.balance').val(formatDecimal(balance));
				tr.find('.balance_').val(balance);
				
				var total_pay = 0;
				$('#tbl_dep .payment_amt').each(function(){
					var parent = $(this).parent().parent();
					var pay_amt_ = parent.find('.payment_amt').val()-0;
					total_pay += pay_amt_;
				});
				...
				$('#tbl_dep .payment_amt').each(function(){
					var parent = $(this).parent().parent();
					var pay_amt = parent.find('.payment_amt').val()-0;
					//var payment = parent.find('.payment_amt').val()-0;
					...
					//alert(total_pay +" | "+ amount_pay);
					if(total_pay != amount_pay) {
						parent.find('.rate').val(formatDecimal(new_rate));
						parent.find('.rate_').val(new_rate);
						parent.find('.percentage').val(formatDecimal(percen));
						parent.find('.percentage_').val(percen);
						parent.find('.total_payment').val(formatDecimal(total_payment));
						parent.find('.total_payment_').val(total_payment);
						parent.find('.balance').val(formatDecimal(balance));
						parent.find('.balance_').val(balance);
					}else{
						if(i == 1) {
							parent.find('.rate').val(formatDecimal(new_rate));
							parent.find('.rate_').val(new_rate);
		...
		$("#date").live('change', function(){
			$("#slref").trigger('change');
		});
		$("#payment_date_new").datetimepicker({
			format: 'yyyy-mm-dd h:i:ss',
			fontAwesome: true,
			language: 'erp',
			weekStart: 1,
		...
		$("#payment_date_new").live('change keyup paste', function(){
			$(".td_date").val($(this).val());
		});
	});
	$(window).load(function () {
		$("#payment_date_new").trigger('change');
    });
...
<script>
$(document).ready(function(){
	$('#reference_nob').val($("#temp_reference_no").val());
	...
	$('#slref').change(function(){
		var get_reference = $(this).val();
		var ref_date = $('#date').val();
		$.ajax({
			url: '<?= base_url() ?>pos/getAjaxReferenceByRef/'+get_reference+'/'+ref_date.replace(' ','_'),
			dataType: 'json',
			success: function(data){
				if(data == 1){
					bootbox.alert("Double Reference Invoice");
	...
	$('#paymentModal').bind('keypress', function (e) {
