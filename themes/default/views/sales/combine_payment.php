...
$(".amount_paid_line").on('change',function() {
    var val = $(this).val();
...
function calculateSum() {
    ...
    $("#amount_1").val(sum.toFixed(2)).trigger('change');
}
...
<div class="modal-dialog">
    <div class="modal-content">
		...
		<?php foreach ($combine_sales->result() as $combine_sale) { 
			if($combine_sale->balance==0 && $combine_sale->payment_status=="paid"){
				
				echo "<div class='modal-body'>";
				echo "<h3>".lang('Payment is already!')."</h3>";
				echo "<p>".lang('Please select the other!')."</p>";
				echo "</div>";
				exit();
			}
		}?>
        ...
        echo form_open_multipart("sales/combine_payment", $attrib); ?>
			...
			<div class="table-responsive">
                <table id="CompTable" cellpadding="0" cellspacing="0" border="0"
                    ...
                    <?php if (!empty($combine_sales)) {
						$total=0;
                        foreach ($combine_sales->result() as $combine_sale) { ?>
                            <tr class="row<?= $combine_sale->id ?> c-foc">
                                <td><?= $combine_sale->date; ?></td>
                                <td><?= $combine_sale->reference_no; ?></td>                           
								<td class="balance"><?= $this->erp->formatMoney($combine_sale->balance); ?></td>
                                <td class="col-xs-3">
									<input type="hidden" name="sale_id[]" class="form-control" value="<?= $combine_sale->id ?>" id="amount_">
                                    <input type="text" name="amount_paid_line[]" class="amount_paid_line form-control" value="<?= $this->erp->formatMoney($combine_sale->balance); ?>">
                                </td>
								<td>
                            ...
                            <div class="col-sm-6">
                                <div class="payment">
                                    <div class="form-group">
                                        ...
                                        <input name="amount_paid" type="text" id="amount_1"
