    ...
    <div class="modal-content">
        <div class="modal-body">
			...
			<?php
                ...
                echo form_open_multipart("sales/delivery_update/" . $did, $attrib)
            ?>
                    ...
                    foreach ($rows as $row): ?>
                        <tr>
										...
										if ($q->num_rows() > 0) {
											foreach ($q->result() as $r) {
												if($r->qty == ""){
													...
													echo form_input('delivery_qty[]', $this->erp->formatQuantity($r->qty), 'class="form-control text-center" id="delivery_qty"  required="required"');
												}
											}
										}
									?>
							...
							<td style="width: 70px; text-align:center; vertical-align:middle;">
								<?php
									$this->db->select('id, created_by, created_at, COALESCE(quantity, 0) as qty');
									$this->db->from('erp_delivery_items');
									$this->db->where('do_reference_no = "'.$delivery->do_reference_no.'" and product_id = "'.$row->product_id.'" ');
									$q = $this->db->get();
									$date = '';
									...
									if ($q->num_rows() > 0) {
										foreach ($q->result() as $r) {
											if($r->qty == ""){
												...
												echo $this->erp->formatQuantity($row->quantity - $r->qty);
											}
											echo '<input type="hidden" value="'.$r->id.'" name="id[]"/>';
											$date = $r->created_at;
											$create_by = $r->created_by;
										}
									}
								?>
								<input type="hidden" value="<?= $date;?>" name="create_at"/>
								<input type="hidden" value="<?= $create_by;?>" name="create_by"/>
							</td>
                        </tr>
                        <?php
                        $r++;
            ...
            <div class="row">
                ...
                <div class="col-xs-3">
                    <?php echo form_submit('update_delivery', lang("update_delivery"), 'id="update_delivery" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;float:right;"'); ?>
                </div>
            </div>
