                            ...
                            <!--<div class="col-sm-6">
                                <div class="discount">
                                    <div class="form-group">
                                        ...
                                        <input name="discount" type="text" id="discount" value="<?= $discount->discount?>" class="pa form-control kb-pad discount"/>
										<?php
											$pr_discount = 0;
											if (isset($discount->discount)) {
												...
												if ($dpos !== false) {
													...
													$pr_discount = (Float) ($pds[0]) / 100;
												} else {
													$pr_discount = $this->erp->formatPurDecimal($discount);
												}
											}
										?>
										<input type="hidden" name="item_discount" value="<?= $pr_discount;?>">
                                    </div>
                                </div>
                            </div>-->
                            <div class="col-sm-6">
                                <div class="payment">
                                    <div class="form-group">
                                        ...
                                        <input name="amount-paid" type="text" id="amount_1" value="<?= $this->erp->formatDecimal($inv->grand_total - $inv->paid) ?>" id="balance" value="<?= $this->erp->formatDecimal($inv->grand_total - $inv->paid - $pr_discount) ?>" class="pa form-control kb-pad amount" required="required"/>
                                    </div>
                                </div>
                            </div>
							<!--<div class="col-sm-6">
                                <div class="balance">
                                    <div class="form-group">
                                        <?= lang("Amount", "balance"); ?>
                                        <input name="balance" type="text" title="<?= $this->erp->formatDecimal($inv->grand_total - $inv->paid) ?>" id="balance" value="<?= $this->erp->formatDecimal($inv->grand_total - $inv->paid - $pr_discount) ?>" class="pa form-control kb-pad balance"/>
                                    </div>
                                </div>
                            </div>-->
							<div class="col-sm-6">
...
<script type="text/javascript" charset="UTF-8">
    $(document).ready(function () {
		...
		$('#amount_1').live('change keyup paste', function(){
			var amount = $(this).val();
			var discount = $('#discount').val();
			var balance = $('#balance').attr('title');
			var final_balance = 0;
			if (/^[a-zA-Z0-9- ]*$/.test(discount) == false) {
				var dis = discount.split('%');
				final_balance = balance - amount - (dis[0]/100);
			}else{
				final_balance = balance - amount - discount;
			}
			$('#balance').val(formatDecimal(final_balance));
		});
		
		$('#discount').live('change keyup paste', function(){
