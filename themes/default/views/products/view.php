        ...
        <script type="text/javascript">
            $(document).ready(function () {
                var oTable = $('#SlRData').dataTable({
                    ...
                    'sAjaxSource': '<?= site_url('reports/getSalesReports/?v=1&product='.$product->id) ?>',
                    ...
                    "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
						...
						nCells[4].innerHTML = currencyFormat(parseFloat(tqty));
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                var oTable = $('#PoRData').dataTable({
                    ...
                    "iDisplayLength": <?= $Settings->rows_per_page ?>,
                    'bProcessing': true, 'bServerSide': true,
                    'sAjaxSource': '<?= site_url('reports/getPurchasesReports/?v=1&product=' . $product->id) ?>',
                    'fnServerData': function (sSource, aoData, fnCallback) {
                        aoData.push({
                            "name": "<?= $this->security->get_csrf_token_name() ?>",
                            "value": "<?= $this->security->get_csrf_hash() ?>"
					...
					{column_number: 4, filter_default_label: "[<?=lang('product_qty');?>]", filter_type: "text", data: []},
