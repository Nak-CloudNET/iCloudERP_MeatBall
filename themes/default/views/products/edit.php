            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->erp->formatDecimal($product->cost) : '')), 'class="form-control tip" id="cost" required="required"') ?>
                    </div>
                ...
                <div class="col-md-6 col-md-offset-1">
					...
					<div class="standard">
						...
						<div class="well well-sm" id="strap-con"
							 style="<?= $this->input->post('related_strap') || count($straps) ? '' : 'display:none;'; ?>">
							<div class="form-group" id="ui" style="margin-bottom: 0;">
								<div class="input-group" style="width:100%;">
									<?php
										...
										echo form_dropdown('related_strap[]', $related_strap, $sp, 'id="related_strap" class="form-control" multiple="multiple"');
									?>
								</div>
								<div style="clear:both;"></div>
							</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        function add_product_item(item) {
            ...
            $.each(items, function () {
                ...
                tr_html += '<td><input class="form-control text-center cb-price" name="combo_item_price[]" type="text" value="' + formatDecimal(this.price) + '" data-id="' + row_no + '" data-item="' + this.id + '" id="combo_item_price_' + row_no + '" onClick="this.select();"><input type="hidden" name="cb_cost[]" value="'+this.cost+'" class="cb-cost"></td>';
			...
			cb_foot.append($('<td style="padding:5px;" colspan="3"><b>Total Cost</b> : <span class="cb_footer_cost"></span></td>'));
        ...
        $(document).on('click', '#updateAttr', function () {
            ...
            //row.children().eq(3).html('<input type="hidden" name="attr_quantity[]" value="' + $('#aquantity').val() + '"><span>' + $('#aquantity').val() + '</span>');
		...
		$(document).on('change', ".qty", function(){
			...
			var cost = $(this).parent().parent().find('.cb-cost').val();
			var qty   = $(this).val();
			var total = price * qty;
			var total_cost = cost * qty;
			//alert(total);total-cost
	...
	function checkComboPrice(){
		...
		$(".cb_footer_cost").text(total_cost);
	}
