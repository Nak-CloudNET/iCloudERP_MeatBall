        ...
        function checkItem(data){
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(data[i].status == 'deduct'){
                    ...
                    qty_deduct = parseFloat(data[i].quantity);                   
                    ...
                    rows = "<tr>"
                        + "<td> <input type='hidden' value='"+data[i].product_id+"' class='convert_from_items_id' name='convert_from_items_id[]' />"
                        + " <input type='hidden' value='"+data[i].product_code+"' class='procode' name='convert_from_items_code[]' />"
                    ...
                    var idd = data[i].product_id;
                    qty_add = parseFloat(data[i].quantity);     
                    ...
                    $(".convert_to_items_id").each(function(){
                        ...
                        var qty = tr.find(".quantity").val();
                        if(idd == itemid){
                           // qty_add  = parseFloat(qty) * 2;
                            $(this).closest('tr').remove();							
                        }
                    });					
                    rows = "<tr>"
                        + "<td> <input type='hidden' value='"+data[i].product_id+"' class='convert_to_items_id' name='convert_to_items_id[]' />"
                        ...
                        + "<td><input type='text' required='required' class='quantity qty_output form-control input-tip' value='"+qty_add+"' name='convert_to_items_qty[]' /><input type='hidden' required='required' class='hide_quantity qty_output form-control input-tip' value='"+qty_add+"' name='hide_convert_to_items_qty[]' /></td>"
                        + '<td><i style="cursor:pointer;" title="Remove" id="1449892339552" class="fa fa-times tip pointer sldel"></i></td>'
                    + "</tr>";
                    $('#tbody-convert-to-items').append(rows);
                }				
			...
			$(".qty_output").keyup(function(){
				...
				var newQty = 0;
				...
				else
					{
						...
						$('.qty_input').each(function() {
							var temp = Number(list[i].qty);
							var newTemp = ((formatPurDecimal(newQty)*temp)/sumQty) + temp;	
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', $convert->reference_no, 'class="form-control input-tip" id="cref"'); ?>
						...
						<div class="col-sm-4">
                            <div class="form-group">
                                ...
                                echo form_dropdown('bom_id', $boms, $convert->bom_id, 'class="form-control" id="bom_id" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("bom") . '"');
                                ?>
                            </div>
                        </div>
						<div class="col-md-12">
										...
										?>
											<tr>
												<td>
													...
													<?php echo $convert_item->product_name . '(' . $convert_item->product_code . ')('. $convert_item->unit . ')'; ?>
												</td>
												<td>
													<select name="convert_from_items_uom[]" class="form-control"></select>
												</td>
												<td>
													<input type="text" required="required" class="quantity form-control input-tip" value="<?php echo $this->erp->formatQuantity($convert_item->quantity); ?>" name="convert_from_items_qty[]" />
												</td>
												<td>
							...
							<div class="controls table-controls">
								<table id="slTable_ "
									...
									<tr>
										...
										<td><input type="text" required="required" class="quantity form-control input-tip" value="<?php echo $this->erp->formatQuantity($convert_item->quantity); ?>" name="convert_to_items_qty[]" /></td>
										<td><i style="cursor:pointer;" title="Remove" id="1449892339552" class="fa fa-times tip pointer sldel"></i></td>
									</tr>
									<?php
											}
                    ...
                    <!-- Button Submit -->
					<div class="col-md-12">
						<div class="fprom-group"><?php echo form_submit('edit_convert', lang("submit"), 'id="bth_convert_items" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
