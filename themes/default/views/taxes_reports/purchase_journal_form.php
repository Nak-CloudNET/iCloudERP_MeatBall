...
<table class="table table-bordered">
	...
	$P7=0;
	  ...
	  if($row->purchase_type==1){
			$P10=$row->amount*$exchange_rate->rate;
			$P11=$row->amount_tax*$exchange_rate->rate;
			$S10 += $P10;
			$S11 += $P11;
			$S13+=$P10;
			$S13+=$P11;
	  ...
	  if($row->purchase_type==2){
		  $P7=$row->non_tax_pur*$exchange_rate->rate;
		  $S7  += $row->non_tax_pur*$exchange_rate->rate;
		  $S13+=$S7;
	  }
	  if($row->purchase_type==3){
		  $P8=($row->value_import*$exchange_rate->rate);
		  $P9=($row->amount_tax*$exchange_rate->rate);
		  $S8  += ($row->value_import*$exchange_rate->rate);
	...
	<tr>
        ...
        <td style="text-align:center !important"  ><?=number_format($row->qty,0);?></td>
        <td style="text-align:right !important"  ><?php if($P7!=0){echo number_format($P7,0);}?></td>
        <td style="text-align:right !important"   ><?php if($P8!=0){echo number_format($P8,0);}?></td>
        <td style="text-align:right !important"   ><?php if($P9!=0){echo number_format($P9,0);}?></td>
        <td style="text-align:right !important"  ><?php if($P10!=0){echo number_format($P10,0);}?></td>
        <td style="text-align:right !important"  ><?php if($P11!=0){echo number_format($P11,0);}?></td>
		<td style="text-align:right !important"   ><?php if($S13!=0){echo number_format($S13,0);}?></td>
    </tr>
	<?php $i++;}?>
    <tr>
        <td style="text-align:right !important"  colspan="6">សរុបទិញជារៀល<br>Total Purchase in KHR</td>
       
        <td style="text-align:right !important" ><?php if($S7!=0){echo number_format($S7,0);}?></td>
        <td style="text-align:right !important" ><?php if($S8!=0){echo number_format($S8,0);}?></td>
        <td style="text-align:right !important" ><?php if($S9!=0){echo number_format($S9,0);}?></td>
        <td style="text-align:right !important" ><?php if($S10!=0){echo number_format($S10,0);}?></td>
        <td style="text-align:right !important" ><?php if($S11!=0){echo number_format($S11,0);}?></td>
        <td style="text-align:right !important" ><?php if($S13!=0){echo number_format($G13,0);}?></td>
    </tr>
    <tr  id="hide-border">
        <td></td>
        <td></td>
